...
var getFullNames = function(objectArray) {
    ...
    for (var i = 0; i < objectArray.length; i++) {
        nameArr = objectArray[i]
        fullNames.push(nameArr.first + " " + nameArr.last)
    }
    return fullNames
}
...
var generateDog = function(inputname){
    var dog = {
        ...
        speak: function(string){
            var dogSpeak = []
            var words = ''
            dogSpeak = string.split(" ")
            
            log(dogSpeak)
            for (var i = 1; i < dogSpeak.length; i++){
            }
        }
    }
    return dog
...
var pluck = function(inputArray, propName) {
    ...
    for (var i = 0; i < inputArray.length; i++) {
        ...
        valuesArray.push(object[propName])
    }
    return valuesArray
...
// use the hasOwnProperty function to tally up after a breakdown
